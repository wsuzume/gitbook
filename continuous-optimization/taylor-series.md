...
また連続最適化は、点$$x ^ k$$からある方向$$m ^ k$$に$$\alpha ^ k$$だけ進んだ点$$x ^ {k+1} = x ^ k + \alpha ^ k m ^ k$$が$$k \to +\infty$$の極限で最適解（または極値）に収束するような手法として構成されることが多いので、ある点の近傍での関数の形状を近似するTaylor展開が連続最適化でよく登場するのである。
