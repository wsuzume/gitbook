...
$$
\left< \Delta x ^ \mathrm{T} H _ {\overline{x}}, \Delta x \right> = \sum _ {i=1} ^ n \sum _ {j=1} ^ n \frac{\partial ^ 2 f (\overline{x})}{\partial x _ i \partial _ j} \Delta x _ i \Delta x _ j \tag{1.1.7}
$$
...
$$
{\rm Hess} f(x) \Delta x = \Delta x ^ \mathrm{T} H _ x \tag{1.1.8}
$$
